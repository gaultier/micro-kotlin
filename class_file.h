...
static const Type *resolver_eval_type(Resolver *resolver, const Type *in) {
  if (in->kind != TYPE_METHOD)
    return in;

  Type *const return_type = &resolver->types.data[in->v.method.return_type_i];

  return resolver_eval_type(resolver, return_type);
}

static bool resolver_is_type_integer(const Type *type) {
  switch (type->kind) {
  case TYPE_BYTE:
  case TYPE_SHORT:
  case TYPE_INT:
...
static bool resolver_are_types_equal(const Resolver *resolver, const Type *lhs,
                                     const Type *rhs) {
  ...
  if (lhs->kind == TYPE_METHOD && rhs->kind == TYPE_METHOD) {
    ...
    if (!resolver_are_types_equal(
            resolver, &resolver->types.data[lhs_method->return_type_i],
...
static u16 resolver_widen_integer_type(const Type *type) {
...
static bool resolver_is_integer_type_subtype_of(const Type *a, const Type *b) {
...
static bool resolver_is_type_subtype_of(Resolver *resolver, Type *a,
                                        const Type *b, Arena scratch_arena,
                                        Arena *arena) {
  // `A <: A` always.
  ...
  case TYPE_INSTANCE: {
    ...
    while (true) {
      ...
      it = &resolver->types.data[it->super_type_i];
...
static u32 resolver_add_type(Resolver *resolver, Type *new_type, Arena *arena);
...
static Str_builder jvm_fill_descriptor_string(Array32(Type) types,
                                              Str_builder sb, u32 type_i,
                                              Arena *arena) {
  pg_assert(type_i < types.len);

  const Type *const type = &types.data[type_i];
  ...
  case TYPE_ARRAY: {
    ...
    return jvm_fill_descriptor_string(types, sb, type->v.array_type_i, arena);
  }
  case TYPE_METHOD:
  case TYPE_CONSTRUCTOR: {
    ...
    return jvm_fill_descriptor_string(types, sb, method_type->return_type_i,
                                      arena);
  }
  default:
    pg_assert(0 && "unreachable");
...
static u32 parser_add_node(Parser *parser, const Ast *node, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(node != NULL);

  *array32_push(&parser->nodes, arena) = *node;
  return array32_last_index(parser->nodes);
}
...
static char *typechecker_type_kind_string(const Array32(Type) types,
                                          u32 type_i) {
  const Type *const type = &types.data[type_i];
...
static Str type_to_human_string(Array32(Type) types, u32 type_i, Arena *arena) {
  const Type *const type = &types.data[type_i];
  ...
  case TYPE_ARRAY: {
    ...
    res = sb_append(
        res, type_to_human_string(types, type->v.array_type_i, arena), arena);
...
static u32 parser_parse_block(Parser *parser, Arena *arena) {
  ...
  const u32 node_i = parser_parse_statements(parser, arena);
...
//     block
//     | statement
static u32 parser_parse_control_structure_body(Parser *parser, Arena *arena) {
  pg_assert(parser);
  pg_assert(arena);
...
static u32 parser_parse_if_expression(Parser *parser, Arena *arena) {
  ...
  const u32 condition_i = parser_parse_expression(parser, arena);
  ...
  const u32 binary_node_i = parser_add_node(parser, &binary_node, arena);
  ...
  return parser_add_node(parser, &if_node, arena);
}
...
//     | 'break'
//     | BREAK_AT
static u32 parser_parse_jump_expression(Parser *parser, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(arena != NULL);
...
static u32 parser_parse_primary_expression(Parser *parser, Arena *arena) {
  ...
  if (parser_match_token(parser, TOKEN_KIND_NUMBER)) {
    ...
    const u32 node_i = parser_parse_expression(parser, arena);
...
static u32 parser_parse_var_declaration(Parser *parser, Arena *arena) {
  ...
  const u32 node_type_i = parser_parse_type(parser, arena);
...
static bool parser_is_lvalue(const Parser *parser, u32 node_i) {
  pg_assert(parser != NULL);
...
static u32 parser_parse_assignment(Parser *parser, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(arena != NULL);
  ...
  u32 lhs_i = parser_parse_expression(parser, arena);
...
//     {NL}
//     (controlStructureBody | ';')
static u32 parser_parse_while_statement(Parser *parser, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(arena != NULL);
  ...
  const u32 node_i = parser_add_node(parser, &node, arena);

  parser->nodes.data[node_i].rhs =
      parser_parse_control_structure_body(parser, arena);

  return node_i;
}

static u32 parser_parse_loop_statement(Parser *parser, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(arena != NULL);
...
static u32 parser_parse_statement(Parser *parser, Arena *arena) {
  ...
  u32 node_i = 0;
  if ((node_i = parser_parse_loop_statement(parser, arena)) != 0)
    return node_i;

  if ((node_i = parser_parse_declaration(parser, arena)) != 0)
    return node_i;

  if ((node_i = parser_parse_assignment(parser, arena)) != 0)
    return node_i;
...
//     'class')
static u32 parser_parse_navigation_suffix(Parser *parser, u32 *main_token_i,
                                          Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(parser->lexer != NULL);
  pg_assert(parser->tokens_i <= parser->lexer->tokens.len);
...
static u32 parser_parse_call_suffix(Parser *parser, u32 *main_token_i,
                                    Arena *arena) {
  if (!parser_match_token(parser, TOKEN_KIND_LEFT_PAREN))
    return 0;
...
//     | indexingSuffix
//     | navigationSuffix
static u32 parser_parse_postfix_unary_suffix(Parser *parser, u32 *main_token_i,
                                             Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(parser->lexer != NULL);
  pg_assert(parser->tokens_i <= parser->lexer->tokens.len);
...
static u32 parser_parse_postfix_unary_expression(Parser *parser, Arena *arena) {
  ...
  parser->nodes.data[rhs_i].lhs = lhs_i;
...
static u32 parser_parse_multiplicative_expression(Parser *parser,
                                                  Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(parser->lexer != NULL);
  pg_assert(parser->tokens_i <= parser->lexer->tokens.len);
...
static u32 parser_parse_generic_call_like_comparison(Parser *parser,
                                                     Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(parser->lexer != NULL);
  pg_assert(parser->tokens_i <= parser->lexer->tokens.len);
...
static u32 parser_parse_statements(Parser *parser, Arena *arena) {
  ...
  u32 node_i = parser_parse_statement(parser, arena);
  if (node_i == 0)
    return node_i;
...
//     ',']] {NL}
//     ')'
static u32 parser_parse_function_value_parameters(Parser *parser,
                                                  Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(parser->lexer != NULL);
  pg_assert(parser->tokens_i <= parser->lexer->tokens.len);
...
//     block
//     | ('=' {NL} expression)
static u32 parser_parse_function_body(Parser *parser, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(arena != NULL);
...
static u32 parser_parse_function_declaration(Parser *parser, Arena *arena) {
  ...
  const u32 fn_i = parser->current_function_i =
      parser_add_node(parser, &node, arena);

  parser_expect_token(parser, TOKEN_KIND_LEFT_PAREN,
                      "expected left parenthesis before the arguments");

  parser->nodes.data[parser->current_function_i].lhs =
      parser_parse_function_value_parameters(parser, arena);

  if (parser_match_token(parser, TOKEN_KIND_COLON)) {
    parser->nodes.data[parser->current_function_i].extra_data_i =
        parser_parse_type(parser, arena);
  }

  parser->nodes.data[parser->current_function_i].rhs =
...
static u32 parser_parse_property_declaration(Parser *parser, Arena *arena) {
  pg_assert(parser != NULL);
  pg_assert(arena != NULL);
...
static u32 parser_parse_declaration(Parser *parser, Arena *arena) {
  ...
  if ((new_node_i = parser_parse_property_declaration(parser, arena)) != 0)
    return new_node_i;
...
static u32 parser_parse(Parser *parser, Arena *arena) {
  ...
  // nodes[0] is a dummy.
  parser->nodes = array32_make(Ast, 1, parser->lexer->tokens.len * 2, arena);

  parser->tokens_i = 1; // Skip the dummy token.

  const u32 root_i = parser_parse_kotlin_file(parser, arena);
...
static u32 resolver_add_type(Resolver *resolver, Type *new_type, Arena *arena) {
  ...
  *array32_push(&resolver->types, arena) = *new_type;
  return array32_last_index(resolver->types);
}
...
static void resolver_load_methods_from_class_file(Resolver *resolver,
                                                  u32 this_class_type_i,
                                                  const Class_file *class_file,
                                                  Arena *arena) {
  pg_assert(resolver != NULL);
  pg_assert(class_file != NULL);
  pg_assert(arena != NULL);
  ...
  for (u64 i = 0; i < pg_array_len(class_file->methods); i++) {
    ...
    if (cli_log_verbose) {
      ...
      LOG("Loaded method %s [%u]: access_flags=%u type=%.*s",
          typechecker_type_kind_string(resolver->types, type_i),
          resolver->types.len, method->access_flags, (int)human_type.len,
          human_type.data);
    }
  }
  ...
  if (has_at_least_one_inline_only_method) {
    ...
    for (u64 i = initial_types_count; i < resolver->types.len; i++) {
      Type *const type = &resolver->types.data[i];
...
static Str resolver_function_to_human_string(const Resolver *resolver,
  ...
  if (!(type->kind == TYPE_METHOD || type->kind == TYPE_CONSTRUCTOR))
    return type_to_human_string(resolver->types, type_i, arena);

  const Method *const method = &type->v.method;

  const Type *const this_class_type =
      &resolver->types.data[method->this_class_type_i];
  ...
  while (tombstones_count < candidates_count - 1) {
    for (u64 i = 0; i < candidates_count; i++) {
      for (u64 j = 0; j < i; j++) {
        ...
        const Type *const b = &resolver->types.data[b_type_i];
...
// TODO: Remove.
static bool typechecker_merge_types(const Resolver *resolver, u32 lhs_i,
                                    u32 rhs_i, u32 *result_i) {
  pg_assert(resolver != NULL);
  pg_assert(lhs_i < resolver->types.len);
  pg_assert(rhs_i < resolver->types.len);
  pg_assert(result_i != NULL);

  const Type *const lhs_type = &resolver->types.data[lhs_i];
  const Type *const rhs_type = &resolver->types.data[rhs_i];
...
static void resolver_ast_fprint_node(const Resolver *resolver, u32 node_i,
  ...
  const Ast *const node = &resolver->parser->nodes.data[node_i];
  ...
  const char *const type_kind =
      typechecker_type_kind_string(resolver->types, node->type_i);
  Str human_type = type_to_human_string(resolver->types, node->type_i, arena);
...
static bool resolver_resolve_fully_qualified_name(Resolver *resolver, Str fqn,
  ...
  for (u64 i = 0; i < resolver->class_path_entries.len; i++) {
    ...
    const u64 previous_len = resolver->types.len;
    jvm_read_jar_file(resolver, class_path_entry, scratch_arena, arena);

    for (u64 i = previous_len; i < resolver->types.len; i++) {
      const Type *const type = &resolver->types.data[i];
      if (type->kind == TYPE_INSTANCE &&
          type_fqn_equal_to_package_and_name(
              fqn, type->package_name, type->this_class_name, scratch_arena)) {
...
static void resolver_load_standard_types(Resolver *resolver, Str java_home,
  ...
  for (u64 i = 0; i < sizeof(known_types) / sizeof(known_types[0]); i++)
    *array32_push(&resolver->types, arena) = known_types[i];
...
static bool typechecker_variable_shadows(Resolver *resolver, u32 name_token_i) {
  ...
  const Ast *const previous_var_node =
      &resolver->parser->nodes.data[previous_var->var_definition_node_i];
...
static u32 resolver_resolve_node(Resolver *resolver, u32 node_i,
  ...
  Ast *const node = &resolver->parser->nodes.data[node_i];
  const Token token = resolver->parser->lexer->tokens.data[node->main_token_i];

  switch (node->kind) {
  case AST_KIND_NONE:
    *array32_push(&resolver->types, arena) = (Type){.kind = TYPE_ANY};

    return node->type_i = array32_last_index(resolver->types);
  case AST_KIND_BOOL: {
  ...
  case AST_KIND_CALL: {
    ...
    const Type *picked_method_type =
        &resolver->types.data[picked_method_type_i];
  ...
  case AST_KIND_UNARY:
    ...
    case TOKEN_KIND_NOT:
      ...
      if (type->kind != TYPE_BOOLEAN) {
        ...
        error = sb_append(
            error, type_to_human_string(resolver->types, node->type_i, arena),
            arena);
  ...
  case AST_KIND_BINARY: {
    ...
    if (!typechecker_merge_types(resolver, lhs_i, rhs_i, &node->type_i)) {
      ...
      error = sb_append(
          error, type_to_human_string(resolver->types, lhs_i, arena), arena);
      error = sb_append_c(error, " vs ", arena);
      error = sb_append(
          error, type_to_human_string(resolver->types, rhs_i, arena), arena);
      parser_error(resolver->parser, token, (char *)error.data);
    }
  ...
  case AST_KIND_VAR_DEFINITION: {
    ...
    if (typechecker_variable_shadows(resolver, node->main_token_i))
      return 0;
    ...
    if (!typechecker_merge_types(resolver, lhs_type_i, rhs_type_i,
                                 &node->type_i)) {
      Str lhs_type_human =
          type_to_human_string(resolver->types, lhs_type_i, arena);
      Str rhs_type_human =
          type_to_human_string(resolver->types, rhs_type_i, arena);
  ...
  case AST_KIND_VAR_REFERENCE: {
    ...
    return node->type_i = resolver->parser->nodes.data[node->lhs].type_i;
  }
  case AST_KIND_IF: {
    pg_assert(node->lhs > 0);
    pg_assert(node->lhs < resolver->parser->nodes.len);
    ...
    if (type_condition->kind != TYPE_BOOLEAN) {
      ...
      Str type_inferred_string =
          type_to_human_string(resolver->types, type_condition_i, arena);
  ...
  case AST_KIND_THEN_ELSE:
    ...
    if (!typechecker_merge_types(resolver, lhs_type_i, rhs_type_i,
      ...
      error = sb_append_c(error, "incompatible types: ", arena);
      error = sb_append(
          error, type_to_human_string(resolver->types, lhs_type_i, arena),
          arena);
      error = sb_append_c(error, " vs ", arena);
      error = sb_append(
          error, type_to_human_string(resolver->types, rhs_type_i, arena),
          arena);
      parser_error(resolver->parser, token, (char *)error.data);
    }
    return node->type_i;
  ...
  case AST_KIND_RETURN: {
    ...
    const Type *const function_type =
        &resolver->types.data[current_function->type_i];
    ...
    if (!resolver_are_types_equal(resolver, &resolver->types.data[node->type_i],
                                  &resolver->types.data[return_type_i])) {
      Str_builder error = sb_new(256, &scratch_arena);
      error =
      ...
      error = sb_append(error,
                        type_to_human_string(resolver->types,
                                             current_function->type_i, arena),
                        arena);
      ...
      error = sb_append(
          error, type_to_human_string(resolver->types, return_type_i, arena),
          arena);
      parser_error(resolver->parser, token, (char *)error.data);
    }
...
static u16 codegen_emit_jump_conditionally(codegen_generator *gen,
  ...
  jvm_code_array_push_u8(&gen->code->code, jump_opcode, arena);
  const u16 jump_from_i = pg_array_len(gen->code->code);
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_IMPDEP1, arena);
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_IMPDEP2, arena);
...
static void codegen_emit_ldc(codegen_generator *gen,
  ...
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_LDC_W, arena);
...
static void codegen_emit_get_static(codegen_generator *gen, u16 field_i,
  ...
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_GET_STATIC, arena);
  jvm_code_array_push_u16(&gen->code->code, field_i, arena);
...
static void codegen_emit_invoke_virtual(codegen_generator *gen,
  ...
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_INVOKE_VIRTUAL, arena);
  ...
  const Type *const return_type =
      &gen->resolver->types.data[method_type->return_type_i];
...
static void codegen_emit_invoke_static(codegen_generator *gen, u16 method_ref_i,
  ...
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_INVOKE_STATIC, arena);
  jvm_code_array_push_u16(&gen->code->code, method_ref_i, arena);
...
static void codegen_emit_invoke_special(codegen_generator *gen,
  ...
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_INVOKE_SPECIAL, arena);
  jvm_code_array_push_u16(&gen->code->code, method_ref_i, arena);
  ...
  while (current < code + code_size) {
    ...
    case BYTECODE_INVOKE_VIRTUAL: {
      ...
      const Type *const invoked_type =
          &gen->resolver->types.data[invoked_type_i];
...
static void codegen_emit_add(codegen_generator *gen, Arena *arena) {
  ...
  switch (kind_a) {
  case VERIFICATION_INFO_INT:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_IADD, arena);
    break;
  case VERIFICATION_INFO_LONG:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_LADD, arena);
    break;
  default:
    pg_assert(0 && "todo");
  }
...
static void codegen_emit_neg(codegen_generator *gen, Arena *arena) {
  ...
  switch (kind) {
  case VERIFICATION_INFO_INT:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_INEG, arena);
    break;
  case VERIFICATION_INFO_LONG:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_LNEG, arena);
    break;
  default:
    pg_assert(0 && "todo");
  }
...
static void codegen_emit_mul(codegen_generator *gen, Arena *arena) {
  ...
  switch (kind_a) {
  case VERIFICATION_INFO_INT:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_IMUL, arena);
    break;
  case VERIFICATION_INFO_LONG:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_LMUL, arena);
    break;
  default:
    pg_assert(0 && "todo");
  }
...
static void codegen_emit_div(codegen_generator *gen, Arena *arena) {
  ...
  switch (kind_a) {
  case VERIFICATION_INFO_INT:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_IDIV, arena);
    break;
  case VERIFICATION_INFO_LONG:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_LDIV, arena);
    break;
  default:
    pg_assert(0 && "todo");
  }
...
static void codegen_emit_rem(codegen_generator *gen, Arena *arena) {
  ...
  switch (kind_a) {
  case VERIFICATION_INFO_INT:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_IREM, arena);
    break;
  case VERIFICATION_INFO_LONG:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_LREM, arena);
    break;
  default:
    pg_assert(0 && "todo");
  }
...
codegen_emit_load_constant_double_word(codegen_generator *gen, u16 constant_i,
  ...
  jvm_code_array_push_u8(&gen->code->code, BYTECODE_LDC2_W, arena);
  jvm_code_array_push_u16(&gen->code->code, constant_i, arena);
...
static void codegen_emit_return_value(codegen_generator *gen, Arena *arena) {
  ...
  switch (kind) {
  case VERIFICATION_INFO_INT:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_IRETURN, arena);
    break;
  case VERIFICATION_INFO_LONG:
    jvm_code_array_push_u8(&gen->code->code, BYTECODE_LRETURN, arena);
    break;
  default:
    pg_assert(0 && "todo");
  }
...
static void codegen_emit_synthetic_if_then_else(codegen_generator *gen,
  ...
  jvm_code_array_push_u8(&gen->code->code, conditional_jump_opcode, arena);
  jvm_code_array_push_u8(&gen->code->code, 0, arena);
  jvm_code_array_push_u8(&gen->code->code, 3 + 2 + 3, arena);
...
static void codegen_emit_node(codegen_generator *gen, Class_file *class_file,
  ...
  pg_assert(gen->resolver->parser->nodes.len > 0);
  pg_assert(class_file != NULL);
  pg_assert(node_i < gen->resolver->parser->nodes.len);
  ...
  const Type *const type = &gen->resolver->types.data[node->type_i];
  ...
  case AST_KIND_CALL: {
    ...
    if (type->flags & TYPE_FLAG_INLINE_ONLY) {
      ...
      descriptor_s = jvm_fill_descriptor_string(
      ...
      const Jvm_constant_pool_entry name_and_type = {
          .kind = CONSTANT_POOL_KIND_NAME_AND_TYPE,
          .v = {.name_and_type = {.name = name_i, .descriptor = descriptor_i}}};
      const u16 name_and_type_i = jvm_constant_array_push(
  ...
  case AST_KIND_FUNCTION_DEFINITION: {
    ...
    descriptor = jvm_fill_descriptor_string(gen->resolver->types, descriptor,
                                            node->type_i, arena);
  ...
  case AST_KIND_VAR_DEFINITION: {
    ...
    const Jvm_verification_info verification_info =
        codegen_type_to_verification_info(
            resolver_eval_type(gen->resolver, type));
    const Jvm_variable variable = {
        .node_i = node_i,
        .type_i = node->type_i,
        .scope_depth = gen->frame->scope_depth,
  ...
  case AST_KIND_VAR_REFERENCE: {
    ...
    pg_assert(node->lhs > 0);
    pg_assert(gen->resolver->parser->nodes.data[node->lhs].kind ==
                  AST_KIND_VAR_DEFINITION ||
              gen->resolver->parser->nodes.data[node->lhs].kind ==
                  AST_KIND_FUNCTION_PARAMETER);
  ...
  case AST_KIND_ASSIGNMENT:
    pg_assert(node->lhs > 0);
    const Ast *const lhs = &gen->resolver->parser->nodes.data[node->lhs];
...
static void codegen_supplement_entrypoint_if_exists(codegen_generator *gen,
  ...
  // Generate code section for the new method.
  {
    ...
    const u16 target_name_and_type_i = jvm_constant_array_push(
    ...
    // Fill locals (method arguments).
    {
      ...
      *array32_push(&gen->resolver->types, arena) = string_type;
      const u32 string_type_i = array32_last_index(gen->resolver->types);

      const Type source_method_argument_types = {
          .kind = TYPE_ARRAY,
          .this_class_name = str_from_c("FIXME"),
          .v = {.array_type_i = string_type_i},
      };
      *array32_push(&gen->resolver->types, arena) =
          source_method_argument_types;

      const u32 source_argument_types_i =
          array32_last_index(gen->resolver->types);
...
static void codegen_emit(Resolver *resolver, Class_file *class_file, u32 root_i,
                         Arena *arena) {
  pg_assert(resolver != NULL);
  pg_assert(class_file != NULL);
  pg_assert(root_i > 0);
  ...
  if (resolver->parser->nodes.len == 1)
    return;
