...
enum __attribute__((packed)) ty_type_kind_t {
  ...
  TYPE_KOTLIN_SHORT = 1 << 5,
  TYPE_KOTLIN_INT = 1 << 6,
  TYPE_KOTLIN_FLOAT = 1 << 7,
  TYPE_KOTLIN_LONG = 1 << 8,
  TYPE_KOTLIN_DOUBLE = 1 << 9,
  TYPE_KOTLIN_STRING = 1 << 10,
...
static bool ty_merge_types(const resolver_t *resolver, u32 lhs_i, u32 rhs_i,
  ...
  // FIXME: Widen.
  if (lhs_type->kind == TYPE_KOTLIN_INT && rhs_type->kind == TYPE_KOTLIN_BYTE) {
    *result_i = rhs_i;
    return true;
  }

  if (lhs_type->kind == TYPE_KOTLIN_BYTE && rhs_type->kind == TYPE_KOTLIN_INT) {
    *result_i = lhs_i;
    return true;
  }
...
static u32 resolver_resolve_node(resolver_t *resolver, u32 node_i,
  ...
  case AST_KIND_LIST: {
    ...
    pg_assert(resolver_resolve_class_name(
        resolver, string_make_from_c_no_alloc("kotlin.Unit"), &node->type_i,
        scratch_arena, arena));
    return node->type_i;
  }
  case AST_KIND_FUNCTION_DEFINITION: {
    ...
    if (node->extra_data_i > 0) {
      ...
      pg_assert(resolver_resolve_class_name(
          resolver, string_make_from_c("kotlin.Unit", arena), &return_type_i,
          scratch_arena, arena));
    }
  ...
  case AST_KIND_WHILE_LOOP: {
    ...
    pg_assert(resolver_resolve_class_name(
        resolver, string_make_from_c("kotlin.Unit", arena), &node->type_i,
        scratch_arena, arena));
    return node->type_i;
  }
