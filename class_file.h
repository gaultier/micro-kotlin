...
jvm_constant_pool_get(Array32(Jvm_constant_pool_entry) constant_pool, u16 i) {
  // 1-indexed.
  pg_assert(i > 0);
...
static void codegen_emit_node(codegen_generator *gen, Class_file *class_file,
  ...
  case AST_KIND_LIST: {
    ...
    // append_frame.
    const Ast *const stmt = ast_handle_to_ptr(node->lhs, *arena);
    if (stmt->kind != AST_KIND_RETURN && // Avoid: `return; pop;`
        gen->frame != NULL) {
